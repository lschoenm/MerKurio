...
- Increased the extract parallel chunk size to `8192` records/pairs to reduce queue and ordering overhead.
- Added `benchmarks/scripts/06-realistic-extract-parallel-benchmarks.sh` for a larger paired-end workload: default `1,000,000` read pairs, `150 bp` reads, `250` deterministic `31 bp` patterns, sparse embedded hits, and `--threads 1/2/4/0`.
