...
The _k_-mers can be provided as a list of strings on the command line or in a file (FASTA or plain text file). The output is written to a SAM or BAM file, depending on the file extension. If the chosen tag is already present in the records, the new values are added to existing ones. Optionally, keep only records which are matching at least one _k_-mer, or keep only records which are not matching any _k_-mer. Parallel matching and tagging are supported for both SAM and BAM files. Record and log order are preserved.
...
| `-p`       | `--threads`          | `<Number of threads>` Maximum total processing threads, including reading and writing, for SAM and BAM. Default is 1 (serial); 0 detects available cores. Additional BAM compression/decompression threads are disabled.                                                                                                                                                                                                                                                    |
|            | `--chunk-size`       | `<Number of records per processing batch>` Default is 1024. Larger batches reduce scheduling overhead but use more memory. |
...
|            | `--hash`         | Use exact rolling-hash matching. All query sequences must have the same length. Hash hits are verified against the query sequence, so this supports arbitrary letters without false-positive matches. Case-insensitive matching is not supported. |
| `-q`       | `--q-size`       | `<Size of q-grams for BNDMq>` Use the Backwards Nondeterministic DAWG Matching algorithm tuned with _q_-grams ([BNDMq](https://doi.org/10.1137/1.9781611972894.3)) with this value for the size of _q_. If omitted, MerKurio selects _q_ automatically based on the pattern length. A manually supplied value must not be larger than the length of the pattern. |

### Parallel processing

Use `--threads N` to set the maximum total thread count. With two threads, one thread reads and tags while the main thread writes. With three or more, one thread reads, `N-2` workers match and tag, and the main thread writes in input order. BAM compression and decompression run within these threads, without additional helpers.

`--chunk-size` controls records per batch (default: 1024; must be positive). Larger batches reduce scheduling overhead but use more memory. Filtering and logging preserve their serial behavior.
